...
public:
  ...
  explicit TSP_GA_mut(std::vector<Coordinates> city_coordinates)
      : m_n_cities(city_coordinates.size()), m_city_coordinates(std::move(city_coordinates)),
  ...
  Population generate(size_t N, RNG &rng)  {
    ...
    std::for_each(population.begin(), population.end(),
                  [&](Individual &i) { std::shuffle(i.begin(), i.end(), rng); });
    return population;
  }

  FitnessMeasure evaluate(const Individual &individual)  {
    // The first city is fixed
    FitnessMeasure total_distance{
        xt::norm_l1(m_city_coordinates[*individual.cbegin()] - m_city_coordinates[0])()};
    for (auto i = individual.cbegin() + 1; i < individual.cend(); i++) {
      total_distance += xt::norm_l1(m_city_coordinates[*i] - m_city_coordinates[*(i - 1)])();
    }
    return 1. / total_distance;
  }

  static Population select_parents(const Population &population, const Evaluations &evaluations,
                                   RNG &rng)  {
    std::discrete_distribution<size_t> parents_distribution(evaluations.cbegin(),
                                                            evaluations.cend());
  ...
  auto crossover(const Individual &parent_1, const Individual &parent_2, RNG &rng)  {
    using std::next;
...
private:
  ...
  void _mutate_reflect(Individual &individual, RNG &rng)  {
    ...
    std::reverse(std::next(individual.begin(), int(i1)), std::next(individual.begin(), int(i2)));
  }
  void _mutate_shift(Individual &individual, RNG &rng)  {
    using std::next;
    std::vector<int> cuts(4);
    std::generate(cuts.begin(), cuts.end(), [&]() { return int(m_cut_distribution(rng)); });
